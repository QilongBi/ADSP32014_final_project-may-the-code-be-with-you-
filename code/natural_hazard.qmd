...
national_working_long <- national_working %>%
  ...
  mutate(response = as.factor(response))  # 如果你想让 response 是 factor 类型
...
results <- lapply(samples, function(smp) {
  ...
  # 计算 coping_appraisal 的边际效应（主效应 + 交互项）
...
# 提取该变量的所有 dummy levels（不包括 baseline）
...
results <- lapply(samples, function(smp) {
  ...
  # 计算 threat_appraisal 的边际效应（主效应 + 交互项）
...
summary_fixed <- subset(fancy_model2_data, 
                         ...
                         !grepl("1\\|id",fancy_model2_data$X)    # 丢掉带 1|id
)
...
summary_fixed <- subset(fancy_model_correct, 
                         ...
                         !grepl("1\\|id",fancy_model_correct$X)    # 丢掉带 1|id
)
